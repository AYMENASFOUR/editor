...
function statsRows(stats: BuildStats): StatRow[] {
  return (
    [
      { icon: MapPin, labelKey: 'loadBuild.stats.sites', count: stats.byType.site ?? 0 },
      { icon: Building2, labelKey: 'loadBuild.stats.buildings', count: stats.byType.building ?? 0 },
      { icon: Layers, labelKey: 'loadBuild.stats.levels', count: stats.byType.level ?? 0 },
      { icon: Square, labelKey: 'loadBuild.stats.walls', count: stats.byType.wall ?? 0 },
      { icon: DoorOpen, labelKey: 'loadBuild.stats.doors', count: stats.byType.door ?? 0 },
      { icon: AppWindow, labelKey: 'loadBuild.stats.windows', count: stats.byType.window ?? 0 },
      { icon: Box, labelKey: 'loadBuild.stats.items', count: stats.byType.item ?? 0 },
      { icon: Square, labelKey: 'loadBuild.stats.slabs', count: stats.byType.slab ?? 0 },
      { icon: Square, labelKey: 'loadBuild.stats.ceilings', count: stats.byType.ceiling ?? 0 },
      { icon: Square, labelKey: 'loadBuild.stats.zones', count: stats.byType.zone ?? 0 },
      { icon: Scan, labelKey: 'loadBuild.stats.scans', count: stats.byType.scan ?? 0 },
    ] satisfies StatRow[]
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {ok ? t('loadBuild.ready') : t('loadBuild.cannotImport')}
          </DialogTitle>
          <DialogDescription>
            {fileName} · {formatFileSize(fileSizeBytes)} ·{' '}
            {t('loadBuild.nodeCount', { count: stats.total })}
          </DialogDescription>
        </DialogHeader>
          ...
          {errors.length > 0 && (
            <div className="space-y-2 rounded-md border border-red-200 bg-red-50 p-3">
              <div className="flex items-center gap-2 font-medium text-red-800 text-sm">
                ...
                {t('loadBuild.errorCount', { count: errors.length })}
              </div>
          ...
          {warnings.length > 0 && (
            <div className="space-y-2 rounded-md border border-amber-200 bg-amber-50 p-3">
              <div className="flex items-center gap-2 font-medium text-amber-800 text-sm">
                ...
                {t('loadBuild.warningCount', { count: warnings.length })}
              </div>
            ...
            <div className="space-y-2 rounded-md border bg-card p-3">
              ...
              >
                ...
                <span className="text-muted-foreground text-xs">
                  {showSchemaIssues ? t('loadBuild.hide') : t('loadBuild.show')}
                </span>
              </button>
              {showSchemaIssues && (
