...
const RAILING_MODE_OPTIONS = (t: TranslateFn): { label: string; value: StairRailingMode }[] => [
  { label: t('np.none'), value: 'none' },
  { label: t('np.left'), value: 'left' },
  { label: t('np.right'), value: 'right' },
  { label: t('np.both'), value: 'both' },
]

const STAIR_TYPE_OPTIONS = (t: TranslateFn): { label: string; value: StairType }[] => [
  { label: t('np.straight'), value: 'straight' },
  { label: t('np.curved'), value: 'curved' },
  { label: t('np.spiral'), value: 'spiral' },
]

const TOP_LANDING_MODE_OPTIONS = (t: TranslateFn): { label: string; value: StairTopLandingMode }[] => [
  { label: t('np.none'), value: 'none' },
  { label: t('np.integrated'), value: 'integrated' },
]

const STAIR_SLAB_OPENING_OPTIONS = (t: TranslateFn): { label: string; value: StairSlabOpeningMode }[] => [
  { label: t('np.none'), value: 'none' },
  { label: t('np.destination'), value: 'destination' },
]
...
export default function StairPanel() {
  ...
  return (
    <PanelWrapper
      ...
      onClose={handleClose}
      title={node.name || t('np.staircase')}
      width={300}
    >
      <PanelSection title={t('np.type')}>
        <SegmentedControl
          ...
          options={STAIR_TYPE_OPTIONS(t)}
          value={node.stairType ?? 'straight'}
        />
      </PanelSection>

      <PanelSection title={t('np.opening')}>
        <div className="space-y-3">
          {attachedDeck ? null : (
            <ToggleControl
              checked={(node.slabOpeningMode ?? 'none') === 'destination'}
              label={t('np.autoCutout')}
            ...
            >
              {levels.map((level) => (
                <option key={level.id} value={level.id}>
                  {level.name || `${t('np.level')} ${level.level + 1}`}
                </option>
              ))}
            </select>
          </div>
          ...
          {attachedDeck ? (
            <div className="space-y-1.5">
              ...
              <SegmentedControl
                ...
                options={[
                  { label: t('np.followsDeck'), value: 'follows' },
                  { label: t('np.customRise'), value: 'custom' },
                ]}
                value={node.totalRise == null ? 'follows' : 'custom'}
              />
              {node.totalRise == null ? (
                <div className="px-1 text-[11px] text-muted-foreground">
                  {t('np.currently')} {resolvedRise} m
                </div>
              ) : (
                <MetricControl
                  label={t('np.rise')}
        ...
        <PanelSection title={t('np.segments')}>
          ...
          <div className="flex gap-1.5">
            <ActionButton
              ...
              label={t('np.addFlight')}
              onClick={handleAddFlight}
            />
            <ActionButton
              icon={<Plus className="h-3.5 w-3.5" />}
              label={t('np.addLanding')}
      ...
      {(node.stairType === 'curved' || node.stairType === 'spiral') && (
        <PanelSection title={t('np.geometry')}>
          <MetricControl
            label={t('np.width')}
            max={10}
            min={0.4}
      ...
      <PanelSection title={t('np.railing')}>
        <SegmentedControl
          onChange={(value) => handleUpdate({ railingMode: value })}
          options={RAILING_MODE_OPTIONS(t)}
