...
import { ActionButton, ActionGroup, PanelSection, triggerSFX, useI18n} from '@pascal-app/editor'
...
export default function DownspoutsPanel() {
  ...
  return (
    <PanelSection title={t('np.downspouts')}>
      <div className="flex flex-col gap-1">
        {downspouts.map((d, i) => (
          ...
          >
            <button
              ...
              type="button"
            >
              {d.name || `${t('np.downspout')} ${i + 1}`}
            </button>
            <button
              aria-label={t('np.removeDownspout')}
        ...
        <ActionGroup>
          <ActionButton label={t('np.addDownspout')} onClick={handleAddDownspout} />
        </ActionGroup>
      </div>
    </PanelSection>
  )
